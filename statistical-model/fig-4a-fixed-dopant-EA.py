"""
Simulation code for figure 4a, DOI:
...
if __name__ == '__main__':
    ...
    for idx, result in enumerate(results):
        ...
        if idx == 0:
            ...
            df.to_csv('data/FIXED-DOP-EA-0p40.csv', sep='\t')
        if idx == 1:
            df = pd.DataFrame(
                x, columns=['N', 'E1', 'E2', 'MR', 'eFermi', 'p', 'q', 'NaNeg'])
            df.to_csv('data/FIXED-DOP-EA-0p45.csv', sep='\t')
        if idx == 2:
            df = pd.DataFrame(
                x, columns=['N', 'E1', 'E2', 'MR', 'eFermi', 'p', 'q', 'NaNeg'])
            df.to_csv('data/FIXED-DOP-EA-0p50.csv', sep='\t')
        if idx == 3:
            df = pd.DataFrame(
                x, columns=['N', 'E1', 'E2', 'MR', 'eFermi', 'p', 'q', 'NaNeg'])
            df.to_csv('data/FIXED-DOP-EA-0p55.csv', sep='\t')

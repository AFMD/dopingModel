"""
Simulation code for figure 4d, DOI:
...
if __name__ == '__main__':
    ...
    for idx, result in enumerate(results):
        ...
        if idx == 0:
            ...
            df.to_csv('data/SHIFTING-DOP-SIGMA-125.csv', sep='\t')
        if idx == 1:
            df = pd.DataFrame(
                x, columns=['N', 'E1', 'E2', 'MR', 'eFermi', 'p', 'q', 'NaNeg'])
            df.to_csv('data/SHIFTING-DOP-SIGMA-150.csv', sep='\t')
        if idx == 2:
            df = pd.DataFrame(
                x, columns=['N', 'E1', 'E2', 'MR', 'eFermi', 'p', 'q', 'NaNeg'])
            df.to_csv('data/SHIFTING-DOP-SIGMA-175.csv', sep='\t')
        if idx == 3:
            df = pd.DataFrame(
                x, columns=['N', 'E1', 'E2', 'MR', 'eFermi', 'p', 'q', 'NaNeg'])
            df.to_csv('data/SHIFTING-DOP-SIGMA-200.csv', sep='\t')

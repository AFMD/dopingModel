"""
Simulation code for Supplementary figure 7, DOI:
...
if __name__ == '__main__':
    ...
    pool = Pool(2)      # Create pool of four workers, same number as CPUs
    # Now create the tasks for workes
    kT = 80 * (Boltzmann / elementary_charge)
    test1 = [iterations, Nrange, kT, E0, sigma, E1, EB, EA]
    kT = 300 * (Boltzmann / elementary_charge)
    ...
    for idx, result in enumerate(results):
        ...
        if idx == 0:
            df = pd.DataFrame(
                x, columns=['N', 'E1', 'E2', 'MR', 'eFermi', 'p', 'q', 'NaNeg', 'NaNegSEP'])
            df.to_csv('data/SHIFTING-DOP-TEMP-80.csv', sep='\t')
        if idx == 1:
            df = pd.DataFrame(
                x, columns=['N', 'E1', 'E2', 'MR', 'eFermi', 'p', 'q', 'NaNeg', 'NaNegSEP'])
            df.to_csv('data/SHIFTING-DOP-TEMP-300.csv', sep='\t')
